  ...
  if (e.keyCode === 13 && e.shiftKey == true) {
    ...
    if (document.getElementById("content").textContent !== '') {
      if (document.getElementById("content").textContent.length > 100) {
        document.getElementById("cantsend").textContent = "100文字以上は送れません";
        cantsend = true;
      } else {
        if (reply_mode) {
          socket.emit("newreply", { "msg": document.getElementById("content").textContent, "reply_id": reply_id })
...
(function () {
  ...
  function setonline() {
    if (houchi) { socket.emit("setstatus", "退席中") } else { socket.emit("setstatus", "オンライン") }
  }
...
fetch("/api/acount").then((res) => {
  res.json().then((text) => {
    ...
    document.getElementById("nowlogin").textContent = `${text.name}/@${text.username}`
  })
})
//<=最初の処理
...
socket.on("newmsg", (msg) => {
  ...
  if (urlcheck(msg.content)) {
    ...
    content_div.innerHTML = `<div id="${msg.id}" class="msg" oncontextmenu="onrightclick('${msg.id}')" onmouseover="onreplybutton('${msg.id}_menu')" onmouseleave="outreplybutton('${msg.id}_menu')"><img class="userimg" src="${msg.img}"><ul class="msg_ul"><li id="${msg.id}_username" class="username">${msg.name}/@${msg.username} <span class="nowdate">${Hour}:${Min}:${Sec}</span></li><br><li id="${msg.id}_content"><a href="${msg.content}" target="_blank" rel="noopener noreferrer" class="info">${msg.content}(外部リンクへ移動します)</a></li></ul><div id="${msg.id}_emoji"></div><div id="${msg.id}_menu" style="display: none;"><button id="${msg.id}_reply" onclick="makereply('${msg.id}')">返信</button> / <button onclick="newemoji('👍','${msg.id}')">👍</button><button onclick="newemoji('🤔' , '${msg.id}')">🤔</button><button onclick="newemoji('👏' , '${msg.id}')">👏</button><button onclick="newemoji('😢' , '${msg.id}')">😢</button></div></div>`;
  ...
  twemoji.parse(document.getElementById("side"));
    ...
    var Min = today.getMinutes();
    var Sec = today.getSeconds();
    content_div.innerHTML = `<div id="${msg.id}" class="msg" oncontextmenu="onrightclick('${msg.id}')" onmouseover="onreplybutton('${msg.id}_menu')" onmouseleave="outreplybutton('${msg.id}_menu')"><img class="userimg" src="${msg.img}"><ul class="msg_ul"><li id="${msg.id}_username" class="username">${msg.name}/@${msg.username}<span class="nowdate">${Hour}:${Min}:${Sec}</span></li><br><li id="${msg.id}_content" class="content">${safeText}</li></ul><div id="${msg.id}_emoji"></div><div id="${msg.id}_menu" style="display: none;"><button id="${msg.id}_reply" onclick="makereply('${msg.id}')">返信</button> / <button onclick="newemoji('👍','${msg.id}')">👍</button><button onclick="newemoji('🤔' , '${msg.id}')">🤔</button><button onclick="newemoji('👏' , '${msg.id}')">👏</button><button onclick="newemoji('😢' , '${msg.id}')">😢</button></div></div>`;
    list.appendChild(content_div, list.firstChild);
    scrollid(msg.id);
    twemoji.parse(document.getElementById("msglist"));
  twemoji.parse(document.getElementById("side"));
...
socket.on("newreply", (msg) => {
  ...
  twemoji.parse(document.getElementById("side"));
    ...
    const reply_name = document.getElementById(msg.reply + "_username").textContent
    const reply_content = document.getElementById(msg.reply + "_content").textContent
    content_div.innerHTML = `<div id="${msg.id}" class="msg" oncontextmenu="onrightclick('${msg.id}')" onmouseover="onreplybutton('${msg.id}_menu')" onmouseleave="outreplybutton('${msg.id}_menu')"><span onclick="scrollid('${msg.reply}')">-->${reply_name}:${reply_content}</span><br><img class="userimg" src="${msg.img}"><ul class="msg_ul"><li id="${msg.id}_username" class="username">${msg.name}/@${msg.username} <span class="nowdate">${Hour}:${Min}:${Sec}</span></li><br><li id="${msg.id}_content" class="content">${safeText}</li></ul><div id="${msg.id}_emoji"></div><div id="${msg.id}_menu" style="display: none;"><button id="${msg.id}_reply" onclick="makereply('${msg.id}')">返信</button> / <button onclick="newemoji('👍','${msg.id}')">👍</button><button onclick="newemoji('🤔' , '${msg.id}')">🤔</button><button onclick="newemoji('👏' , '${msg.id}')">👏</button><button onclick="newemoji('😢' , '${msg.id}')">😢</button></div></div>`;
    list.appendChild(content_div, list.firstChild);
    scrollid(msg.id);
    twemoji.parse(document.getElementById("msglist"));
  twemoji.parse(document.getElementById("side"));
...
function check_height() {
  document.getElementById("msglist").style.height = window.innerHeight - document.getElementById('msgsend').offsetHeight + "px";
}
//<=高さ調整
...
function check_mamber() {
  fetch("/api/chat/nowlogin").then((res) => {
    res.json().then((text) => {
      if(!text.nowlogin.includes(username_g)){
        if (!stopserver) {
          swal("サーバーから切断されました!", "リロードをして、再接続してください。", "error").then((willDelete) => {
            location.reload()
          })
          stopserver = true;
        }
  ...
  for (var i = 0; i < content.length; i++) {
    const content_div = document.createElement("div");
    const list = document.querySelector("#online");
    content_div.innerHTML = `<div><img src=""><span class="status_name">${content[i]}</span><br><span class="status" id="${content[i]}"></span><div>`
    list.appendChild(content_div, list.firstChild);
  }
      }
    })
